...
/**
 * Deploys all necessary stacks for the OversightML Test Imagery within the specified AWS CDK application.
 *
 * @param vpcStack An instance of `OSMLVpcStack` representing the VPC configuration to be used by the test imagery stack.
 */
export function deployTestImagery(
  ...
  const imageryStack = new OSMLTestImageryStack(
    ...
    `${appConfig.projectName}-Test-Imagery`,
    {
      ...
      description:
        "OSML Test Imagery, Guidance for Processing Overhead Imagery on AWS (SO9240)"
    }
  );
